  ...
  var ROTINA_TIPOS=[["Reel","Reels",""],["Carrossel","Carrossel",""],["Imagem","Post estático",""],["Vídeo","Vídeo longo",""]];
  ...
  function rotinaCardHtml(){
    ...
    var head='<div class="bt">Rotina de publicação e gravação</div>';
  ...
  function agendaHtml(items){
    ...
    A.keys.forEach(function(k){
      ...
      if(wk!==lastWeek){if(lastWeek)h+='</div>';h+='<div class="semana"><div class="semana-t">Semana de '+("0"+mon.getDate()).slice(-2)+'/'+("0"+(mon.getMonth()+1)).slice(-2)+'</div>';lastWeek=wk;}
      var D=A.days[k],rows='';
      ...
      h+='<div class="ag-day"><div class="ag-d">'+esc(DIAS_SEM_LONGO[d.getDay()])+' <span>'+("0"+d.getDate()).slice(-2)+'/'+("0"+(d.getMonth()+1)).slice(-2)+'</span></div>'+rows+'</div>';
    });
    return h+'</div>';
  }
