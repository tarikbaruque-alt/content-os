  ...
  function buildBriefingFormHtml(cfg){
    ...
    var data=JSON.stringify({mark:BRIEF_MARK,de:de,wa:wa,mail:mail,fields:BRIEF_FIELDS,envio:cfg.envio||null}).replace(/</g,"\\u003c");
    var css='*{box-sizing:border-box}body{margin:0;font:16px/1.5 system-ui,-apple-system,Segoe UI,Roboto,sans-serif;background:#F9F9FD;color:#2A2A2A}main{max-width:640px;margin:0 auto;padding:22px 16px 60px}h1{font-size:22px;margin:0 0 6px}.sub{color:#5f5d6e;font-size:14px;margin:0 0 18px}.card{background:#fff;border:1px solid #e6e4ef;border-radius:14px;padding:16px;margin-bottom:14px}h2{font-size:13px;letter-spacing:.06em;text-transform:uppercase;color:#4D4AEA;margin:22px 0 10px}label.q{display:block;font-weight:600;font-size:14.5px;margin-bottom:6px}.req{color:#c0392b}input[type=text],textarea{width:100%;border:1px solid #d9d6e6;border-radius:10px;padding:11px 12px;font:inherit;background:#fbfbfe}textarea{min-height:84px;resize:vertical}.opts{display:flex;flex-wrap:wrap;gap:8px}.opts label{display:flex;align-items:center;gap:7px;border:1px solid #d9d6e6;border-radius:999px;padding:8px 13px;font-size:14px;cursor:pointer;background:#fbfbfe}.opts input{accent-color:#4D4AEA}.btn{display:block;width:100%;border:0;border-radius:12px;padding:15px;font:600 16px system-ui,sans-serif;cursor:pointer;margin-top:10px}.pri{background:#4D4AEA;color:#fff}.wa{background:#1fa855;color:#fff}.sec{background:#F4F3FF;color:#3E3BC4}.msg{font-size:13.5px;color:#5f5d6e;text-align:center;margin-top:10px;min-height:20px}.err{color:#c0392b}#out{display:none}.fim{text-align:center;padding:64px 8px}.fim h1{font-size:26px}';
    var js='(function(){var D='+data+';var F=D.fields,box=document.getElementById("form"),KEY="cos_briefing_rascunho";'+
      ...
      'function mostrarOut(){document.getElementById("out").style.display="block";document.getElementById("txt").value=texto();document.getElementById("out").scrollIntoView({behavior:"smooth"})}'+
      // Link do painel: as respostas vão direto para a equipe. Se a rede falhar, cai no envio manual.
      'function enviar(){var bt=document.getElementById("go"),m=document.getElementById("msg"),o={};F.forEach(function(f){var v=val(f);if(v)o[f.k]=v});bt.disabled=true;bt.textContent="Enviando";'+
      'fetch(D.envio.url,{method:"POST",headers:{"Content-Type":"application/json",apikey:D.envio.anon,Authorization:"Bearer "+D.envio.anon},body:JSON.stringify({op:"briefing_enviar",token:D.envio.token,respostas:o})})'+
      '.then(function(r){return r.json().then(function(j){if(!r.ok)throw new Error((j&&j.error&&j.error.message)||"Não consegui enviar.");})})'+
      '.then(function(){try{localStorage.removeItem(KEY)}catch(e){}document.querySelector("main").innerHTML="<div class=fim><h1>Recebemos o seu briefing</h1><p class=sub>Obrigado! "+(D.de?esc(D.de)+" já pode ver":"A equipe já pode ver")+" as suas respostas e começa a montar a sua estratégia de conteúdo. Pode fechar esta página.</p></div>";window.scrollTo(0,0)})'+
      '.catch(function(e){bt.disabled=false;bt.textContent="Concluir briefing";m.className="msg err";m.textContent=e.message+" Suas respostas continuam salvas neste aparelho. Tente de novo ou use Copiar respostas abaixo.";mostrarOut()})}'+
      'document.getElementById("go").addEventListener("click",function(){if(!ok())return;if(D.envio)enviar();else mostrarOut()});'+
      'var em=document.getElementById("em");if(em){if(!D.mail||D.envio)em.style.display="none";em.addEventListener("click",function(){var nm=(box.querySelector("[data-k=name]")||{}).value||"";var u="mailto:"+D.mail+"?subject="+encodeURIComponent("📋 Briefing de conteúdo, "+nm.trim())+"&body="+encodeURIComponent(texto());window.location.href=u})}'+
      'var wa=document.getElementById("wa");if(D.envio&&!D.wa)wa.style.display="none";if(!D.wa)wa.textContent="Enviar pelo WhatsApp";wa.addEventListener("click",function(){var u="https://wa.me/"+(D.wa||"")+"?text="+encodeURIComponent(texto());window.open(u,"_blank")});'+
    ...
    return '<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Briefing de conteúdo'+(para?', '+esc(para):de?', '+esc(de):'')+'</title><style>'+css+'</style></head><body><main>'+
      ...
      '<p class="sub" style="background:#F4F3FF;color:#3E3BC4;border-radius:12px;padding:12px 14px"><b>Quanto mais detalhes você colocar, mais personalizadas ficam a sua estratégia e as ideias de conteúdo.</b> Responda com calma, exemplos reais, palavras que seus clientes usam e histórias ajudam muito.</p>'+
