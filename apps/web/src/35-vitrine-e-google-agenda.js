  ...
  function vtStatusHtml(it){return it.clientStatus==="aprovado"?'<span class="vt-st ok">✓ Aprovada</span>':it.clientStatus==="ajuste"?'<span class="vt-st adj">✏️ Ajuste pedido</span>':'';}
  function vitrineHtml(items,mode){
    ...
    var h='<div class="vt" data-vt-mode="'+mode+'"><header class="vt-hero"><div class="vt-emoji">'+esc(emoji)+'</div>'+
      ...
      (mode==="painel"?'<button class="vt-edit" data-vt-edit>✏️ Editar topo</button>':'')+'</header>'+
    ...
    ord.forEach(function(o){var it=o.it,x=it.idea||{},id="pauta-"+(it.id||o.idx),d=parseD(it.data),mon=new Date(d.getTime());mon.setDate(d.getDate()-((d.getDay()+6)%7));var wk=fmtD(mon);
      ...
      h+='<details class="vt-p" id="'+id+'" data-vt-item="'+esc(it.id||"")+'" data-vt-status="'+esc(it.clientStatus||"")+'" data-vt-label="'+esc(dataBR(it.data)+" "+itemHora(it,r)+" — "+hd)+'">'+
        ...
        '<div class="vt-actions"><button class="vt-btn ok" data-vt-ok>✅ Aprovar pauta</button><button class="vt-btn" data-vt-adj>✏️ Pedir ajuste</button></div>'+
  ...
  function vtFileScript(D){
    ...
    function paint(){var o=load(),n=0,tot=0;[].forEach.call(document.querySelectorAll(".vt-p"),function(p){tot++;var s=o[p.getAttribute("data-vt-item")]||(p.getAttribute("data-vt-status")?{st:p.getAttribute("data-vt-status")}:null),ch=p.querySelector(".vt-chips"),old=ch.querySelector(".vt-st");if(old)old.remove();
      if(s){if(s.st==="aprovado")n++;var b=document.createElement("span");b.className="vt-st "+(s.st==="aprovado"?"ok":"adj");b.textContent=s.st==="aprovado"?"✓ Aprovada":"✏️ Ajuste pedido";ch.appendChild(b)}});
  ...
  function periodSelectorHtml(){
    ...
    return '<div class="card pad" style="margin-bottom:16px"><div class="bt" style="font-size:11px;font-weight:600;text-transform:uppercase;letter-spacing:.06em;color:var(--faint);margin-bottom:10px">Planejamento editorial por período</div>'+
      ...
      '<button class="btn pri genbtn" id="genPlanBtn" style="margin-left:auto">✦ Gerar Planejamento Editorial</button><span id="planMsg" style="font-size:12px;color:var(--muted)"></span></div>'+
      '<div style="font-size:11.5px;color:var(--faint);margin-top:8px">Gerar de novo substitui o calendário atual deste cliente (os horários e o dia de gravação ficam na Rotina).</div></div>';
  }
  function renderCal(){
    var col={Reel:"#5B45E6",Carrossel:"#2E6FB7",Stories:"#0E8C9B"};
    ...
    if(isDbClient(state.client)){
      ...
      var h='<div class="section-head" style="margin-top:6px"><div><h3>Calendário editorial — '+esc(clientName(state.client))+'</h3><p>Operado por <b>Cronos</b>. Clique num conteúdo para ver o contexto estratégico e gerar o roteiro/copy.</p></div></div>'+rotinaCardHtml()+periodSelectorHtml();
      if(items.length)h+=exportBarHtml();
      h+=items.length?(state.calMode==="lista"?calListHtml(items):agendaHtml(items)):('<div class="card empty-hero"><div class="eh-ic">▦</div><div class="eh-t">'+(hasIdeas?'Escolha o período acima e gere o planejamento.':'Gere as <b>Ideias</b> primeiro.')+'</div></div>');
      el.innerHTML=h;
      Array.prototype.forEach.call(el.querySelectorAll('.periodBtn'),function(b){b.addEventListener('click',function(){state.period=+b.getAttribute('data-days');renderCal();})});
      function setDias(d){state.diasPost=d.slice().sort();state.diasPostCli=state.client;renderCal();}
      Array.prototype.forEach.call(el.querySelectorAll('.ppwBtn'),function(b){b.addEventListener('click',function(){setDias(DIAS_PADRAO[+b.getAttribute('data-n')]);})});
      Array.prototype.forEach.call(el.querySelectorAll('.diaBtn'),function(b){b.addEventListener('click',function(){var d=diasPostOf(),v=+b.getAttribute('data-dia'),i=d.indexOf(v);if(i>=0)d.splice(i,1);else d.push(v);setDias(d);})});
      var gp=I("#genPlanBtn");if(gp)gp.addEventListener('click',function(){runGerarPlanejamento(state.period||30);});
      if(items.length){wireGen('.view[data-view="calendar"] [data-gen]');wireCalExtras(items);}else wireRotina();
      return;
    }
    if(genOn()){
