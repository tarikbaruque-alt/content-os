  ...
  function renderCobrancaBanner(){var el=I("#ovCobranca");if(!el)return;if(state.clientView){el.innerHTML="";return;}
    ...
    el.innerHTML='<div class="card pad" style="margin-top:16px;display:flex;gap:12px;align-items:center;flex-wrap:wrap;border-color:transparent;background:var(--hyp-bg)"><span style="font-size:20px">💸</span><div style="flex:1;min-width:200px"><b>Lembrete de cobrança</b><div style="font-size:12.5px;color:var(--muted)">'+p.length+' cliente(s) com vencimento já passado este mês ainda não marcado(s) como recebido — '+brl(tot)+'.</div></div><button class="btn pri" data-goto="ativos">Ver cobranças →</button></div>';
  ...
  function renderAtivos(){
    ...
    var h='<div class="section-head" style="margin-top:6px"><div><h3>Clientes ativos <span class="tag-mock">só você vê</span></h3><p>Com quem você está trabalhando, contatos, valores e cobranças. Nada disto aparece para o cliente.</p></div><button class="btn pri" id="atNovo">+ Novo cliente</button></div>'+
      '<div class="grid cols-3" style="margin-bottom:16px"><div class="card kpi"><span class="k-lbl">Clientes ativos</span><div class="k-val tnum" style="font-size:40px">'+ativos.length+'</div><div class="k-sub">'+(inat?inat+' inativo(s) com histórico guardado':'nenhum inativo')+'</div></div>'+
      '<div class="card kpi"><span class="k-lbl">Receita mensal (ativos)</span><div class="k-val tnum">'+brl(receita)+'</div><div class="k-sub">'+(comValor.length?'média de '+brl(receita/comValor.length)+' por cliente':'cadastre o valor de cada cliente')+'</div></div>'+
      '<div class="card kpi"><span class="k-lbl">Recebidos em '+MESES_LONGOS[new Date().getMonth()]+'</span><div class="k-val tnum">'+pagos+'<span style="font-size:16px;color:var(--faint)"> / '+comValor.length+'</span></div><div class="k-sub">'+(pend.length?pend.length+' cobrança(s) pendente(s)':'nada pendente até hoje')+'</div></div></div>';
    if(pend.length){h+='<div class="card pad" style="margin-bottom:16px;background:var(--hyp-bg);border-color:transparent"><div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:8px"><b>💸 Cobranças do mês</b><span style="font-size:12px;color:var(--muted)">vencimento já passou (dia '+pend.map(function(c){return adminOf(c).venc}).filter(function(v,i,a){return a.indexOf(v)===i}).join(", ")+') e ainda não marcado como recebido</span><button class="btn ghost" id="atTpl" style="margin-left:auto">✏️ Mensagem de cobrança</button></div>'+
      pend.map(function(c){var a=adminOf(c);return '<div class="ag-row" style="cursor:default;align-items:center;border-color:rgba(0,0,0,.06)"><div style="flex:1;min-width:0"><b>'+esc(c.name)+'</b> · '+brl(a.valor)+' <span class="ag-sub" style="display:inline">· vence dia '+a.venc+'</span></div>'+
        (a.whats?'<button class="btn" data-at-cobrar="'+esc(c.id)+'">💬 Cobrar no WhatsApp</button>':'<button class="btn" data-at-copiar="'+esc(c.id)+'">📋 Copiar mensagem</button>')+'<button class="btn pri" data-at-pago="'+esc(c.id)+'">✓ Recebido</button></div>'}).join('')+'</div>';}
    h+='<div style="display:flex;gap:6px;flex-wrap:wrap;align-items:center;margin-bottom:12px">'+[["ativos","Ativos ("+ativos.length+")"],["inativos","Inativos ("+inat+")"],["todos","Todos ("+todos.length+")"]].map(function(f){return '<button class="preset atFiltro" data-f="'+f[0]+'" style="'+(filtro===f[0]?'background:var(--brand-weak);color:var(--brand-ink);border-color:transparent':'')+'">'+f[1]+'</button>'}).join('')+
      '<span style="flex:1"></span><button class="btn" id="atGcal" title="Cria um lembrete que se repete todo mês na sua Google Agenda">📆 Lembrete mensal de cobrança na Google Agenda</button></div>';
    h+=lista.length?'<div class="grid cols-3">'+lista.map(function(c){var a=adminOf(c),f=c.ficha||{},ig=String(f.instagram||"").trim(),brief=!!String(c.briefing||"").trim();
      return '<div class="card pad at-card" data-at-open="'+esc(c.id)+'" style="cursor:pointer'+(a.ativo?'':';opacity:.62')+'"><div style="display:flex;gap:10px;align-items:center;margin-bottom:10px"><div style="width:40px;height:40px;border-radius:11px;display:grid;place-items:center;color:#fff;font-family:var(--font-display);font-weight:700;background:'+avc((byId(c.id)||{}).av||0)+'">'+esc(String(c.name||"?").charAt(0))+'</div><div style="min-width:0;flex:1"><div style="font-family:var(--font-display);font-weight:700;font-size:15px;line-height:1.2">'+esc(c.name)+'</div><div style="font-size:12px;color:var(--muted)">'+esc(c.niche||"Área não informada")+'</div></div>'+
        '<span class="badge '+(a.ativo?'act':'')+'">'+(a.ativo?'Ativo':'Inativo')+'</span></div>'+
        '<div style="font-size:12.5px;line-height:1.9">'+(ig?'📸 '+esc(ig.charAt(0)==="@"?ig:"@"+ig):'<span style="color:var(--faint)">📸 sem @</span>')+'<br>'+(a.whats?'💬 <a href="'+waLink(a.whats)+'" target="_blank" rel="noopener" data-stop>'+esc(a.whats)+'</a>':'<span style="color:var(--faint)">💬 sem WhatsApp</span>')+'<br>💰 <b>'+(a.valor?brl(a.valor)+'/mês':'<span style="color:var(--faint);font-weight:400">sem valor</span>')+'</b>'+(a.valor?' <span style="color:var(--faint)">· vence dia '+a.venc+(a.pagos[mes]?' · ✓ recebido':'')+'</span>':'')+'</div>'+
        '<div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:12px">'+(brief?'<span class="badge act">✓ briefing</span>':'<button class="btn pri" data-at-brief="'+esc(c.id)+'">+ Adicionar briefing</button>')+'<button class="btn" data-at-edit="'+esc(c.id)+'">✏️ Editar</button><button class="btn ghost" data-at-toggle="'+esc(c.id)+'">'+(a.ativo?'Marcar inativo':'Reativar')+'</button></div></div>';}).join('')+'</div>':
      '<div class="card empty-hero"><div class="eh-ic">＋</div><div class="eh-t">'+(filtro==="inativos"?'Nenhum cliente inativo.':'Nenhum cliente ainda. Cadastre ou importe um briefing em <b>Clientes</b>.')+'</div></div>';
    el.innerHTML=h;
    I("#atNovo").addEventListener('click',openNewClientModal);
  ...
  function renderClientePagina(el,id){
    ...
    var h='<div style="margin:4px 0 14px"><button class="btn ghost" id="cpVoltar">← Clientes ativos</button></div>'+
      ...
      '<div style="text-align:right"><div style="font-family:var(--font-display);font-weight:700;font-size:20px">'+(a.valor?brl(a.valor)+'<span style="font-size:13px;color:var(--faint)">/mês</span>':'—')+'</div><div style="font-size:12px;color:var(--muted)">'+(a.valor?'vence dia '+a.venc+(a.pagos[mesChave()]?' · ✓ recebido este mês':''):'')+'</div></div><button class="btn" id="cpEdit">✏️ Editar dados</button></div>';
    ...
    h+=sec("Briefing",bBody+'<div id="cpBriefEdit" hidden style="margin-top:10px"><textarea class="ta" id="cpBriefTxt" style="min-height:160px" placeholder="O que o cliente vende, público, dores, desejos, objeções, diferencial, provas, tom de voz, objetivos…">'+esc(brief)+'</textarea><div style="display:flex;gap:8px;margin-top:8px;align-items:center"><button class="btn pri" id="cpBriefSalvar">Salvar briefing</button><span id="cpBriefMsg" style="font-size:12px;color:var(--muted)"></span></div></div>',
      brief?'<button class="btn" id="cpEscrever2">✏️ Editar</button><button class="btn" id="cpColar2">📥 Importar outro</button>':'');
  ...
  function openNewClientModal(){
    ...
    var html='<div class="scrim" id="scrim"></div><aside class="drawer" role="dialog" style="width:min(440px,100%)"><div class="dh"><div class="d-title">Novo cliente</div><button class="icon-btn" id="dclose">✕</button></div><div class="db">'+
      '<div class="block"><div style="font-size:12.5px;color:var(--muted);margin-bottom:14px">Começa 100% em branco — sem Linha Editorial, ideias, copies ou roteiros de exemplo. Você preenche o Content DNA (manualmente ou com a Íris) e gera cada etapa quando quiser. Tudo que você preencher fica gravado — não precisa digitar de novo.</div>'+
      ...
      '<div style="font-size:11.5px;color:var(--faint);margin:4px 0 12px">Opcional — quanto mais você preencher agora, mais ricos ficam todos os agentes (dá pra completar depois na Ficha do cliente):</div>'+
