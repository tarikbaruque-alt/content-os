  ...
  function renderFormats(){
    ...
    var h='<div class="section-head" style="margin-top:6px"><div><h3>Formatos — '+esc(clientName(state.client))+'</h3><p><b>Musa</b> — '+(cv?'os formatos que mais funcionam no seu nicho, por que funcionam e como fazer.':'o guia de formatos para o nicho deste cliente: o que priorizar, em que proporção, com que produção e o que evitar.')+'</p></div></div>';
    h+='<div class="card pad"><div class="eyebrow">Nicho</div><div style="font-family:var(--font-display);font-weight:700;font-size:19px;margin-top:4px">'+esc(g.perfil.nome)+(g.ia?' <span class="fit fit-cc" style="vertical-align:middle">personalizado pela IA</span>':'')+'</div>'+
      (cv?'':'<div style="font-size:12px;color:var(--muted);margin-top:4px">'+origem+'</div>')+
      '<div style="font-size:13.5px;margin-top:12px;line-height:1.55">'+esc(g.leitura)+'</div>'+
      (cv?'':'<div style="display:flex;gap:10px;flex-wrap:wrap;align-items:center;margin-top:14px"><label style="font-size:12px;color:var(--muted)">Trocar nicho <select id="fmtNiche" class="btn" style="margin-left:6px">'+opts+'</select></label>'+
        (canAi?'<button class="btn pri genbtn" id="fmtAiBtn">✦ Personalizar com IA</button>':'')+
        '<span id="fmtMsg" style="font-size:12px;color:var(--muted)"></span></div>')+'</div>';
    h+='<div class="section-head"><div><h3>Formatos recomendados</h3><p>Em ordem de prioridade. Carro-chefe = a base do perfil; apoio = complementa; pontual = de vez em quando.</p></div></div>';
    h+='<div class="clist">'+g.formatos.map(function(f,i){
      return '<div class="card pad fmt-rank"><div class="fmt-n">'+(i+1)+'</div><div style="min-width:0"><div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap"><b style="font-family:var(--font-display);font-size:14.5px">'+esc(f.nome)+'</b>'+fitBadge(f.papel)+'</div>'+
        '<div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:7px"><span class="badge">'+esc(f.superficie)+'</span><span class="badge">'+esc(f.producao)+'</span></div>'+
        '<div style="font-size:12.5px;margin-top:10px;color:var(--muted)"><b style="color:var(--ink)">Por quê:</b> '+esc(f.porque)+'</div>'+
        '<div style="font-size:12.5px;margin-top:6px;color:var(--muted)"><b style="color:var(--ink)">Como fazer:</b> '+esc(f.comoFazer)+'</div></div></div>';
    }).join('')+'</div>';
    var sup=g.superficies,seg=sup.map(function(s,i){return '<div class="seg" style="flex:'+s.pct+' 1 0;background:'+avc(i)+'">'+esc(s.superficie)+' '+s.pct+'%</div>'}).join('');
    h+='<div class="clist" style="margin-top:14px"><div class="card pad"><div class="eyebrow" style="margin-bottom:10px">Mix de superfícies</div><div class="stack">'+seg+'</div>'+
      '<div style="font-size:12.5px;margin-top:12px"><b>Produção:</b> '+esc(g.producao.nivel)+' — <span style="color:var(--muted)">'+esc(g.producao.porque)+'</span></div>'+
      '<div style="font-size:12.5px;margin-top:6px"><b>Cadência:</b> <span style="color:var(--muted)">'+esc(g.cadencia)+'</span></div></div>'+
      '<div class="card pad"><div class="eyebrow" style="margin-bottom:10px">Séries / quadros recorrentes</div><div style="display:flex;gap:7px;flex-wrap:wrap">'+g.series.map(function(s){return '<span class="badge">'+esc(s)+'</span>'}).join('')+'</div>'+
      '<div style="font-size:12px;color:var(--muted);margin-top:10px">Quadros fixos criam hábito: mesmo nome, mesma abertura, mesmo dia da semana.</div></div></div>';
    h+='<div class="clist" style="margin-top:14px"><div class="card pad"><div class="eyebrow" style="margin-bottom:8px;color:var(--warn)">Evitar</div>'+g.evitar.map(function(e){return '<div style="font-size:12.5px;margin-top:8px"><b>'+esc(e.pratica)+'</b> — <span style="color:var(--muted)">'+esc(e.porque)+'</span></div>'}).join('')+'</div>'+
      '<div class="card pad"><div class="eyebrow" style="margin-bottom:8px;color:var(--warn)">Cuidados éticos e regulatórios</div>'+g.cuidados.map(function(c){return '<div style="font-size:12.5px;margin-top:8px;color:var(--muted)">• '+esc(c)+'</div>'}).join('')+'</div></div>';
    if(!cv)h+=perfCardHtml();
    h+='<div class="section-head"><div><h3>Todas as opções</h3><p>A biblioteca completa, com o encaixe de cada formato neste nicho. “Livre” = pode usar quando a ideia pedir.</p></div></div>';
    h+='<div class="fmt-grid">'+g.opcoes.map(function(o){
      return '<div class="card pad" style="padding:13px 14px"><div style="display:flex;gap:6px;align-items:center;justify-content:space-between"><b style="font-size:13px">'+esc(o.nome)+'</b>'+fitBadge(o.fit)+'</div>'+
        '<div style="font-size:11.5px;color:var(--faint);margin-top:4px">'+esc(o.superficie)+' · '+esc(o.producao)+'</div>'+
        '<div style="font-size:12px;color:var(--muted);margin-top:6px">'+esc(o.descricao)+'</div>'+
        (o.fit==="Com cuidado"&&o.nota?'<div style="font-size:11.5px;color:var(--warn);margin-top:6px">'+esc(o.nota)+'</div>':'')+'</div>';
    }).join('')+'</div>';
    h+='<div style="font-size:11.5px;color:var(--faint);margin-top:16px">Heurísticas de mercado para começar — ajuste com os dados reais de Performance. Regras de conselhos mudam: confira a versão vigente.</div>';
    el.innerHTML=h;
  ...
  function renderAnalyze(){
    ...
    el.innerHTML='<div class="section-head" style="margin-top:6px"><div><h3>Análise IA — Íris <span class="statuspill lvl-func" style="vertical-align:middle">Funcional</span></h3><p>Cole um briefing e a Íris estrutura o Content DNA <b>ao vivo</b>, com a mesma lógica coberta por testes. Nada vira fato sem sua aprovação.</p></div></div>'+
  ...
  function runDist(){
    ...
    var warn=r.warnings.length?'<div class="callout" style="margin-top:14px;background:var(--hyp-bg)"><span>⚠️</span><div class="em" style="color:var(--warn)">'+r.warnings.map(esc).join('<br>')+'</div></div>':'';
  ...
  function renderDistribution(){
    ...
    el.innerHTML='<div class="section-head" style="margin-top:6px"><div><h3>Distribuição Editorial <span class="statuspill lvl-func" style="vertical-align:middle">Funcional</span></h3><p>Quantos conteúdos no mês e o mix por funil. O plano é calculado de verdade (método do maior resto) — competência de <b>Cronos + Bússola</b>.</p></div></div>'+
  ...
  function renderPlan(){
    ...
    var h='<div class="section-head" style="margin-top:6px"><div><h3>Plano do Mês — '+esc(g.clientName)+' <span class="statuspill lvl-func" style="vertical-align:middle">Funcional</span></h3><p>Gerado <b>ponta a ponta</b> pelo pipeline (Íris → Estratégia → Editorial → Ideias → Produção → Calendário → Notion), provider <b>'+esc(g.provider)+'</b>. Prosa final publicável depende do Anthropic.</p></div></div>';
    // Estratégia
  ...
  function carouselHtml(cr,x,idx){
    ...
    var slides=(cr.slides||[]).map(function(s,i){return '<div class="stp"><div class="sl">'+esc(s.n+' · '+s.papel)+'</div><div class="sv"><b'+(db?' contenteditable="true" data-slide-t="'+i+'"':'')+'>'+esc(s.titulo)+'</b><div style="color:var(--muted);margin-top:3px"'+(db?' contenteditable="true" data-slide-x="'+i+'"':'')+'>'+esc(s.texto)+'</div><div style="font-size:11.5px;color:var(--faint);margin-top:5px">🎬 '+esc(s.visual)+'</div><div style="font-size:11.5px;color:var(--faint);margin-top:2px">🖼️ busca: '+esc(s.imagem)+'</div></div></div>'}).join('');
